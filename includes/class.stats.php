  ...
  class dlStats {
    ...
    // API: Dash League Stats
      public function stats_api( &$api ) {
        ...
        $api[] = array(
          'namespace' => 'api/v1',
          'route'     => array('stats', 'boost'),
          'args'      => array(
            array(
              'methods'             => WP_REST_Server::CREATABLE,
              'callback'            => array($this, 'stats_boost'),
              'permission_callback' => array($this, 'api_authenticate')
            ),
          ),
        );
      ...
      public function stats_boost() {
        global $pxl, $wpdb;
        
        $response = array();
        
        $team    = get_post($_POST['team']);
        $player  = isset($_POST['player']) ? get_post($_POST['player']) : FALSE;
        $players = !$player ? get_field('players', $team->ID) : array($player);
        
        $data = array(
          'season'    => $pxl->season['number'],
          'game_id'   => 'boost - late entry',
          'team_id'   => $team->ID,
          'team'      => $team->post_title,
          'datetime'  => date('Y-m-d H:i:s'),
          'rank_gain' => $_POST['mmr_boost'],
        );
        
        foreach($players as $player) {
          $dl_player = array_merge(array(
            'name'      => $player->post_title,
            'player_id' => $player->ID,
          ), $data);
          
          $wpdb->insert('dl_players', $dl_player);
        }
      ...
      public function stats_forfeit() {
        ...
        foreach ($teams as $side => $args) {
          $team = $args['team']; $opponent = $args['vs'];
          
          $dl_player = array(
            'team_id'     => $team->ID,
            'team'        => $team->post_title,
            'opponent_id' => $opponent->ID,
            'opponent'    => $opponent->post_title,
            'outcome'     => $side == 'opponent',
          );
          
          foreach ($args['players'] as $key => $player) {
            $gamer_ids = get_field('gamer_ids', $player->ID);
            $gamer_id  = !empty($gamer_ids) ? $gamer_ids[0]['id'] : NULL;
            
            $player_data = array_merge($data, $dl_player, array(
              'gamer_id'  => $gamer_id,
              'name'      => $player->post_title,
              'player_id' => $player->ID,
              'rank_gain' => $forfeit_type == 'match' || $forfeit_type == 'double-forfeit' ? 110 : 45, // changed from 200/80 to match averages
            ));
            
            $wpdb->insert('dl_players', $player_data);
          }
        }
          ...
          if ( is_array($args['players']) ) {
            foreach ($args['players'] as $player) {
              ...
              if ( $id = $this->player_id($player) ) {
                $rank = 1000;
                $rank += $wpdb->get_var($wpdb->prepare("SELECT SUM(rank_gain) FROM dl_players WHERE player_id = %d AND season = %d GROUP BY player_id", $id, $pxl->season['number']));
                
                $player['player_id'] = "{$id}|{$rank}";
    ...
    // Helpers
      ...
      public function team( $team_id ) {
        ...
        $sql = "
          SELECT r1.name, sum(r1.MMR) as mmr, count(r1.Matches) as matches, sum(r1.wins) as won, count(r1.Matches) - sum(r1.wins) as lost, ROUND(SUM(r1.kills)/SUM(r1.deaths), 2) as kd, SEC_TO_TIME(SUM(time_to_sec(r1.time))) as time_played
          FROM (
            ...
            WHERE t1.season = 3 and t1.team_id = '{$team_id}'
            GROUP BY t1.team_id, t1.matchID
            ORDER BY name ASC
          ) as r1
          GROUP BY r1.name
